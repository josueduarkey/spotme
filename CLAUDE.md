...
- **"Mi aventura" — roadmap personalizado** (`app/aventura.tsx`, generador en `generarAventura()` de `lib/queries/routes.ts`): 2 preguntas (intereses multi-chip de las 7 categorías + tiempo en lenguaje de viajero: Escapada/Fin de semana/Una semana/Sin prisa) → itinerario día a día con lugares reales agrupados por cercanía (vecino más próximo desde San Salvador), presupuesto por día (entradas + $0.15/km) y botón "Trazar ruta del día" que abre `ruta.tsx` (que ahora acepta `stopIds` CSV como paradas precargadas). El CTA del Home pasó de "Planificar tu ruta" a "Arma tu aventura" → `/aventura`; la selección manual en mapa (`planificar-ruta`) sigue disponible como enlace secundario. La card "Ver el mapa" usa el logo `assets/diorama-sv.png`. `COSTO_CATEGORIA` ahora se exporta de `routes.ts` (única fuente).
