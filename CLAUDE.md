...
- **Admin como entidad aparte** (`constants/admins.ts` con la allowlist; cuenta demo eduardo.garcia@keyinstitute.edu.sv creada y verificada E2E): al iniciar sesión (o con sesión persistida) un admin va **directo a su área** `app/(admin)/` con tabs propios — **Dashboard** (4 stat tiles + gráficas SVG: línea de fotos/día 7 días, barras por categoría, apiladas oficial/comunidad y turistas/negocios; paleta validada CVD con el skill dataviz, `react-native-svg` ya estaba en el APK), **Reportes** (general en texto + 2 CSV, generados con datos vivos y compartidos vía Share nativo) y **Usuarios** (todas las cuentas con tipo/puntos/fecha). El gate vive en `(admin)/_layout.tsx`; no-admins reciben Redirect. Charts en `components/graficas.tsx`, queries en `lib/queries/admin.ts`. Es gate de UI (las tablas leídas ya son públicas por RLS); si algún día expone datos sensibles, mover a `is_admin` + RLS.
...
- **"Mi aventura" — roadmap personalizado por ZONAS** (`app/aventura.tsx`, generador en `generarAventura()` de `lib/queries/routes.ts`): 2 preguntas (intereses multi-chip de las 7 categorías + tiempo: Escapada/Fin de semana/Una semana/Sin prisa) → itinerario con filosofía **profundidad sobre amplitud**: 1-3 días = UNA sola zona (el cluster con más densidad de lugares que hacen match) a fondo; 5 días = 2 zonas; 7+ = hasta 3, encadenadas por cercanía. **Máximo 2 lugares por día** (decisión del usuario: no saturar). Presupuesto por día (entradas + $0.15/km), botón "Trazar ruta del día" → `ruta.tsx` (acepta `stopIds` CSV como paradas precargadas). Cada zona se **enriquece con Google Places API (New) `searchNearby`** (`nearbyPlaces()` en `search.ts`, misma key ya activa, tipos mapeados por categoría con fallback defensivo a tourist_attraction): las sugerencias aparecen como "También cerca de tu zona" y al tocarlas abren "Crear lugar" con lat/lng precargados — Google alimenta el mapa comunitario, no lo reemplaza. El CTA del Home pasó a "Arma tu aventura" → `/aventura`; selección manual (`planificar-ruta`) sigue como enlace secundario. La card "Ver el mapa" usa el logo `assets/diorama-sv.png`. `COSTO_CATEGORIA` se exporta de `routes.ts` (única fuente).
- **Insights reubicado (decisión de producto 2026-07-12)**: la Inteligencia Territorial es para **negocios y admin, NO turistas** — se quitó del perfil turista; el acceso vive en el dashboard de negocio (card "tu próxima oportunidad") y en el dashboard admin (enlace al final). `/insights` sin gate propio (acceso solo por navegación; PoC).
- **Detalle de eventos** (`app/evento/[id].tsx` + `getEventById`): fecha completa, galería "Así se ha vivido" (fotos locales de ediciones anteriores en `constants/fotosEventos.ts` — ⚠️ solo referenciar archivos existentes, un require() roto tumba el bundle de Metro; para agregar: poner el .jpg en `assets/` y añadir la línea), **ubicación exacta** en mini-mapa con pin (tap → `/como-llegar`) y coordenadas. `TarjetaEvento` ahora navegable desde el Home. Fotos reales de "Torneo de Surf El Tunco" (`assets/surf-city.jpg`) y "Feria del Añil" (`assets/feria-anil.jpg`) ya cargadas — los 4 eventos seed tienen galería.
- **Card "Ver el mapa"**: sin nota inferior, más grande (diorama 175px, título 28), fondo claro con peana (se quitó el azul tinta).
- **💰 MODELO DE NEGOCIO — "Reservar mi experiencia" (feature final, 2026-07-12)**: Spotmi se vende como **guía turístico digital**: el itinerario de "Mi aventura" se convierte en un paquete todo-arreglado que el turista (especialmente extranjero) reserva — guía local + transporte + entradas + soporte. Ingresos = **comisión de servicio 12%** por reserva (`SERVICIO_PCT` en `lib/experiencia.ts`) + guía $35/día compartido. Flujo: aventura generada → CTA "Reservar esta experiencia · desde $X/persona" → `app/reservar.tsx` (selector 1-8 personas, desglose transparente, "qué incluye") → "Solicitar reserva" guarda el itinerario en `planned_routes` y envía la solicitud por **WhatsApp** al equipo de guías (`WHATSAPP_RESERVAS` en `lib/experiencia.ts` — ⚠️ placeholder 50370000000, poner número real del equipo). Sin pasarela de pago en el PoC (honesto: "pagas al confirmar con tu guía"). Handoff de datos vía `lib/aventuraActual.ts` (buzón en memoria, no params). API de lugares: **Google Places API (New) es la fuente** (searchText + searchNearby, ya activa en la key) — no existe API pública del MITUR; OpenTripMap/Foursquare casi vacíos en SV.
- **Fix 3D crítico — cámara bajo tierra**: las alturas de cámara en Cesium son ELIPSOIDALES (absolutas); el Centro Histórico está a ~660 m s.n.m. y la cámara nacía a 430 m → enterrada (pantalla negra / vista desde abajo). Corregido: vista del Centro a 1200 m abs (pitch -35°), "Volar a un Destino" a 3200 m abs (sobre El Pital, 2730 m, el punto más alto del país). `CENTRO_HISTORICO` actualizado a las coordenadas oficiales del equipo (13.697497, -89.190313).
- **Gemelo 3D migrado de Cesium a Mapbox GL JS (decisión del usuario 2026-07-12, supera la nota anterior de "no Mapbox")**: el tab "3D Real" ahora renderiza **Mapbox GL JS v3.7 estilo Standard** (edificios 3D + terreno con relieve `mapbox-terrain-dem-v1` exageración 1.35 + niebla) dentro del **mismo WebView** — NO es @rnmapbox/maps nativo, sigue siendo JS puro sin rebuild y Expo Go en iPhone sigue vivo. Paridad completa: 4 chips (Lugares/Negocios/Actividad/Edificios 3D vía `setConfigProperty('basemap','show3dObjects')`), marcadores HTML clickeables → ficha, círculos de actividad (capa GeoJSON), buscador → fly_to, mira central + "Crear aquí" (`map.getCenter()`, más confiable que el pickPosition de Cesium), destinos destacados, listener de mensajes en window+document (Android). Token público en `EXPO_PUBLIC_MAPBOX_TOKEN` (validado contra la API, 200); sin token, el tab muestra instrucciones. La var `EXPO_PUBLIC_CESIUM_ION_TOKEN` quedó como legado sin uso.

**Fase 6 — social + UGC ampliado (2026-07-12, Cuenta A):** ⚠️ **Requiere correr el bloque `FASE 6` al final de `schema.sql` en el SQL Editor de Supabase** (idempotente).

- **Eventos comunitarios** (`app/crear-evento.tsx`, entrada "+ Agregar evento" en el Home): mismo modelo de confianza que lugares — pin en mapa → fecha/hora con chips (30 días, sin date-picker nativo → sin rebuild) → nombre/descripción/foto opcional. `events` ganó `source/created_by/verification_count/is_verified`; `event_verifications` + trigger a 3 confirmaciones; RLS anti-autoconfirmación. `createEvent`/`confirmEvent` en `events.ts`; el detalle de evento muestra badge Nuevo/Verificado + caja "¿Este evento es real?" y usa `cover_image_url` como portada.
- **Rating de lugares**: columna `rating` (1-5) en `place_verifications`; `confirmPlace(id, rating?)` lo envía desde el selector de estrellas de la caja "¿Estuviste aquí?"; `getPlaceRating()` promedia y la ficha lo muestra junto al departamento (★ 4.5 (3)).
- **Comentarios con reacciones** (`components/Comentarios.tsx`, `lib/queries/comments.ts`, tablas `comments`+`comment_reactions`): planos (sin respuestas), corazón toggle (1 por usuario, unique), optimistic UI. Montado en ficha de lugar/negocio y en detalle de evento.
- **Avatar**: tocar el avatar del perfil abre galería (crop 1:1), sube a `uploads/avatars/` y guarda `profiles.avatar_url` (`updateAvatar`/`getAvatarUrl` en `profile.ts`).
- **Leaderboard comunitario** (tab "Ranking" con Trophy en la barra del turista; `app/(turista)/ranking.tsx` + `getLeaderboards()` en `lib/queries/leaderboard.ts`): 4 métricas conmutables — Puntos totales, Lugares creados, Eventos publicados, Visitas (lugares DISTINTOS fotografiados) — con podio oro/plata/bronce (avatares reales), fila propia resaltada "(tú)", top 20, solo cuentas turista. **Nuevo trigger `handle_new_event` (+40 pts por evento de comunidad)** y la reconciliación de puntos se movió al FINAL de `schema.sql` (referencia columnas que FASE 6 crea) e incluye el término de eventos — ⚠️ re-correr `schema.sql` en el SQL Editor para activar los +40 y reconciliar saldos.
- **Insights legibles**: `/insights` ahora abre con 3 tarjetas "Insights clave" en lenguaje plano (departamentos con turistas activos, % del mapa nacido de la comunidad, departamentos "dormidos" con lugares sin fotos) + texto que explica qué mide la barra (demanda = fotos). Feedback del usuario: las barras solas no se entendían.
- **TikTok** (`lib/tiktok.ts`): deep link a la app (`snssdk1233://search?keyword=`) con fallback web; cards "Ver en TikTok" en ficha ("{lugar} El Salvador") y en evento (ediciones pasadas). Logo real en `components/IconoTikTok.tsx` (paths del SVG del equipo vía react-native-svg).
- Todo JS puro — llega por Metro sin rebuild del APK.
