...
- **Detalle de eventos** (`app/evento/[id].tsx` + `getEventById`): fecha completa, galería "Así se ha vivido" (fotos locales de ediciones anteriores en `constants/fotosEventos.ts` — ⚠️ solo referenciar archivos existentes, un require() roto tumba el bundle de Metro; para agregar: poner .jpg en `assets/eventos/` y descomentar la línea), **ubicación exacta** en mini-mapa con pin (tap → `/como-llegar`) y coordenadas. `TarjetaEvento` ahora navegable desde el Home. Pendiente del equipo: fotos reales para "Torneo de Surf El Tunco" y "Feria del Añil".
