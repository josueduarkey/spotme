...
- **Edificios 3D reales — Cesium OSM Buildings (2026-07-12)**: nueva capa togglable "Edificios 3D" en `(turista)/gemelo-3d.tsx` — volúmenes reales por edificio (huella + altura de OpenStreetMap, `Cesium.createOsmBuildingsAsync()`), cobertura mundial. Complementa el mallado fotorrealista de Google (cuya resolución fuera de ciudades insignia puede ser más plana) con geometría real por edificio, notable sobre todo al acercarse al Centro Histórico. Requiere `EXPO_PUBLIC_CESIUM_ION_TOKEN` en `.env` (cuenta gratuita en ion.cesium.com/tokens, sin tarjeta) — sin ella, la capa no se ofrece (el chip no aparece) y el resto del gemelo 3D sigue igual; JS puro, sin rebuild del APK. ⏳ **Pendiente del equipo: crear el token y agregarlo al `.env`** para activar la capa.
