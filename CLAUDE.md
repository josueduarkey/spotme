...
- Pendiente de Fase 3 original: la **pantalla 8** (UI de subir foto a lugar existente) — el backend ya está listo: `uploadPhoto(photoUri, lat, lng, targetId?, targetType?)` en `lib/queries/uploads.ts` sube a Storage y asocia al lugar/negocio más cercano (máx. 15 km; si no hay nada cerca, sugiere crear el lugar — devuelve `targetName` para el mensaje de éxito). También `getPhotosFor(targetType, targetId)` para la galería de la ficha. Ambas verificadas contra la DB real.
